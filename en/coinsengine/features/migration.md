...
A plugin from which you plan to migrate **must be installed** on the server during migration process. You'd also need a [currency](currencies.md#creation) to migrate in.
...
:::caution
If you get a currency error when migrating from Vault, ensure the currency used for migration is **not** linked with [Vault](../hooks/vault.md).
:::
