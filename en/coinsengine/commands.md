...
* `/coe create <name> <symbol> <decimals>` - [Creates](features/currencies.md#creation) a new currency.
...
* `/coe migrate <plugin> <currency>` - [Migrate](features/migration.md) balances from specific plugin to specific currency.

## Currency Commands

Each currency has it's own command aliases defined in the [currency config](features/currencies.md) under `Command_Aliases` option.
...
* `/coins top [page]` - Top players with "coins". Requires the [Leaderboards](features/leaderboards.md) feature to be enabled.
...
* `/wallet [player]` - View balance of all currencies. Requires the [Wallet](features/wallet.md) feature to be enabled.
* `/balance [player]` - View balance of primary currency.
* `/baltop [page]` - Top players by primary currency. Requires the [Leaderboards](features/leaderboards.md) feature to be enabled.
* `/pay <player> <amount>` - Pay player with primary currency.
* `/paytoggle [player]` - Toggle primary currency payments from players.

:::note
Primary currency is the one that linked with [Vault](hooks/vault.md).
:::
